...
class FilmForm extends Component {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if(film == undefined) {
            ...
            // api.films.fetchAll()
            // .then(films =>   this.setState({data: films[0]}))
           // // let regExr = /(\/films\/edit\/)([A-Za-z\d]+)/g;
            // //    console.log(window.location.pathname)
            ////    let resultID = window.location.pathname.replace(regExr, `$2`) 
            return {
                data: initialData,
                error: {},
            }
        ...
        // api.films.fetchAll()
        // .then(films => films = filmOne)
        // .then(films => console.log(films[0]._id))
        // .then(films =>  filmOne = films[0]._id)
